"""Interactive Brokers (IBKR) adapter: ``ib_insync`` session and order helpers.

Implements the project's broker surface: ``connect``, ``disconnect``,
``download_daily``, ``download_halfhours``, ``positions``, ``openOrders``,
``prepare_order``, ``place_order``, ``cancel_order``, ``get_trade_status``.

The name ``IBClient`` in ``sellmanagement.ib_client`` is an alias of ``IBKRBroker``
...
class IBKRBroker:
    ...
    def download_daily(self, token: str, duration: str = "1 Y") -> List[Dict[str, Any]]:
        ...
        if not self._use_ib or self._ib is None:
            raise RuntimeError("IBKRBroker.download_daily requires a live ib_insync connection")
    ...
    def download_halfhours(self, token: str, duration: str = "31 D", end: str | None = None) -> List[Dict[str, Any]]:
        ...
        """
        if not self._use_ib or self._ib is None:
            raise RuntimeError("IBKRBroker.download_halfhours requires a live ib_insync connection")
    ...
    def positions(self) -> List[Any]:
        if not self._use_ib or self._ib is None:
            raise RuntimeError("IBKRBroker.positions requires a live ib_insync connection")
        return self._ib.positions()

    def openOrders(self) -> List[Any]:
        if not self._use_ib or self._ib is None:
            raise RuntimeError("IBKRBroker.openOrders requires a live ib_insync connection")
    ...
    def prepare_order(self, token: str, quantity: int, order_type: str = "MKT", **kwargs) -> Dict[str, Any]:
        ...
        """
        if not self._use_ib or self._ib is None:
            raise RuntimeError("IBKRBroker.prepare_order requires a live ib_insync connection")
    ...
    def place_order(self, prepared_or_token, quantity: int | None = None, order_type: str | None = None, transmit: bool = True, **kwargs) -> Dict[str, Any]:
        ...
        """
        if not self._use_ib or self._ib is None:
            raise RuntimeError("IBKRBroker.place_order requires a live ib_insync connection")
    ...
    def cancel_order(self, order_or_trade) -> Dict[str, Any]:
        ...
        """
        if not self._use_ib or self._ib is None:
            raise RuntimeError("IBKRBroker.cancel_order requires a live ib_insync connection")
